...
export function AddChildModal({ isOpen, onClose, onSuccess, parentId }: AddChildModalProps) {
  ...
  // Fallback function to generate access code client-side
  ...
  const onSubmit = async (data: AddChildFormData) => {
    ...
    try {
      ...
      // Try to generate access code via Edge Function first
      ...
      if (codeError || !codeData?.code) {
        ...
        console.warn('Edge function failed, using client-side code generation:', codeError?.message);
        
        // Generate code client-side and check for uniqueness
        let isUnique = false;
        let attempts = 0;
        ...
        while (!isUnique && attempts < maxAttempts) {
          ...
          if (checkError && checkError.code === 'PGRST116') {
            ...
            // Other error, but assume code is unique
            isUnique = true;
            accessCode = generatedCode;
          }
        }
